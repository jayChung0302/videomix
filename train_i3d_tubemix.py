...
def run(mode_dir, split_path, split, root, resume, init_lr=0.1, max_steps=args.steps, mode='opt',
    ...
    optimizer = optim.SGD(i3d.parameters(), lr=lr, momentum=0.9, weight_decay=0.0000001)  # 0.0000001
        ...
        for phase in ['train', 'val']:
            ...
            for data in dataloaders[phase]:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    inputs, labels = data
                    if phase == 'train':
                        inputs, labels= tubemix(inputs, labels, args.alpha, args.prob)
